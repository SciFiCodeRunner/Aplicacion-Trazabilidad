		...
		if ($request){
			...
			if ($query!='') {
				...
				->leftJoin('vehiculo_transporte_material as vtmd','vtmd.id_abscisa_cargue','=',
					'abs.idAbscisa')
				->select('abs.idAbscisa','abs.nombre','abs.volumen_llenado_teorico','abs.volumen_excavado_teorico',DB::raw('sum(vtm.cantidadMaterial) as volumenLlenado'),DB::raw('sum(vtmd.cantidadMaterial) as volumenExcavado'))
				->where('abs.idAbscisa','LIKE','%'.$query.'%')
				->where('abs.estadoAbscisa','=',1)
				->orderBy('abs.nombre','asc')
				->groupBy('abs.idAbscisa','abs.nombre','abs.volumen_llenado_teorico','abs.volumen_excavado_teorico','volumen_real_llenado','volumen_real_excavado') 
				->paginate(1000) ;
				...
				->groupBy('abs.nombre','vtm.fecha','vtm.numeroRecibo','vht.placa','vtm.cantidadMaterial','mat.nombre') 
				->paginate(1000);
				return view('traza.abscisas.index2',["abscisa2"=>$abscisa2,"abscisa3"=>$abscisa3,"abscisa1"=>$abscisa1,"searchText"=>$query]);
			}else{
				$abscisas= DB::table('abscisas as abs')
				->leftJoin('vehiculo_transporte_material as vtm','vtm.id_abscisa_descargue','=','abs.idAbscisa')
				->leftJoin('vehiculo_transporte_material as vtmd','vtmd.id_abscisa_cargue','=',
					'abs.idAbscisa')
				->select('abs.idAbscisa','abs.nombre','abs.volumen_llenado_teorico','abs.volumen_excavado_teorico',DB::raw('sum(vtm.cantidadMaterial) as volumenLlenado'),DB::raw('sum(vtmd.cantidadMaterial) as volumenExcavado'),'volumen_real_llenado','volumen_real_excavado')
				->where('abs.idAbscisa','LIKE','%'.$query.'%')
				->where('abs.estadoAbscisa','=',1)
				->orderBy('abs.nombre','asc')
				->groupBy('abs.idAbscisa','abs.nombre','abs.volumen_llenado_teorico','abs.volumen_excavado_teorico','volumen_real_llenado','volumen_real_excavado') 
